          ...
          success: function(data){
            ...
            for( x in data.data ){
              if(data.data[x].type == "video"){
                $("#images").append('<video   id = "video'+i+'" class="col-md-6 col-lg-4 my-auto" width="320" height="320" controls><source src="'+data.data[x].videos.standard_resolution.url+'" type = "video/mp4" >  Votre navigateur ne supporte pas les vidéos. </video>');
              }else
                $("#images").append('<img id = "image'+i+'" class="col-md-6 col-lg-4 my-auto" src="'+data.data[x].images.low_resolution.url+'">');
